...
class LookingUpRecipient extends SendState {
  const LookingUpRecipient();
}

class PreparingUpload extends SendState {
...
class RecipientFound extends SendState {
  ...
  @override
  List<Object?> get props => [displayCode, recipientUid];
}
...
class FilesSelected extends SendState {
  ...
  @override
  List<Object?> get props => [files, isMetered];
}

class Uploading extends SendState {
  const Uploading({required this.fileProgress, required this.totalProgress});
  final Map<String, double> fileProgress; // fileId -> 0.0..1.0
  final double totalProgress;
  @override
  List<Object?> get props => [fileProgress, totalProgress];
}

class UploadComplete extends SendState {
...
/// Upload is paused because the app was backgrounded/removed from recents.
class UploadPaused extends SendState {
  const UploadPaused({required this.transferId});
